...
import { SYMPTOMS, ORGAN_SYMPTOMS, SUGGESTIONS, SYMPTOM_OPTIONS } from '../data/symptoms';
...
// Expected rendered height of the embedded 2-column symptom-cards grid (3
// rows of 220px cards + 2 row gaps + the grid's own top/bottom padding). The
// BodySvg wrapper next to it uses this same fixed number instead of
// `alignSelf: 'stretch'`, since stretching to an indeterminate sibling
// height was producing a runaway gap below the row.
const SYMPTOM_GRID_H = 220 * 3 + 2 * 16 + 2 * 16;
...
const EMPTY_FORM = { name: '', sensorId: null, symptoms: new Set(), note: '' };
...
function BodySvg({ stretch }) {
  return (
    <svg
      ...
      style={stretch
        ? { height: '95%', width: 'auto', pointerEvents: 'none' }
        : { height: '100%', width: 'auto', aspectRatio: '198.1 / 466.3', pointerEvents: 'none' }}
    >
      <path
...
function SymptomCards({ categoryLevels, lang, embedded, columns }) {
  const L = lang === 'it';
      ...
      {Object.entries(ZONE_CATS).map(([key, z]) => {
        ...
        return (
          <div key={key} style={embedded
            ...
            {noSym ? (
              <div style={{ fontFamily: 'var(--font-title)', fontSize: 13, lineHeight: 1.5, color: 'rgba(255,255,255,0.85)' }}>
...
export default function SymptomsPage({ lang, liveRows, timeControl, sensorControl, reportsControl, embedded, hideHero, hideMatrixReport, hideReport }) {
  ...
  // When `reportsControl` is passed (lifted to App.jsx so Archive's qualitative
  // table can read the same list), submissions are shared app-wide instead of
  // living only in this component.
  const [reportsInternal, setReportsInternal] = useState([]);
  const reports   = reportsControl ? reportsControl.reports   : reportsInternal;
  const addReport = reportsControl ? reportsControl.addReport : (entry) => setReportsInternal((prev) => [entry, ...prev]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.name.trim() || !form.sensorId || !form.symptoms.size) return;
    const sensor = SENSORS.find((s) => s.id === form.sensorId);
    const now = new Date();
    addReport({
      id: Date.now(),
      ...
      dateObj: now,
      dateStr: now.toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric' }),
      hourStr: `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`,
      name: form.name.trim(),
  ...
  const validForm = form.name.trim() && form.sensorId && form.symptoms.size > 0;
    ...
    <div className="symptoms-page">
      ...
      <div style={embedded ? undefined : {
        ...
        <div style={embedded ? {
          ...
          backgroundImage: "url('/assets/cielo.png')", backgroundSize: 'cover', backgroundPosition: 'center',
        } : {
          display: 'flex', alignItems: 'stretch', borderBottom: '1px solid var(--gray)',
        ...
        <div style={embedded ? {
          flex: '1 1 50%', minWidth: 0, height: SYMPTOM_GRID_H, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 32, boxSizing: 'border-box',
        } : {
          flex: 1, height: '90%', alignSelf: 'center', position: 'relative', overflow: 'hidden', background: 'var(--white)', display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <BodySvg stretch={embedded} />
        </div>

        <SymptomCards categoryLevels={categoryLevels} lang={lang} embedded={embedded} columns={embedded} />
        </div>
      </div>
      </>
      )}
      ...
      <div id="report-section" style={{ borderTop: '1px solid var(--gray)' }}>
        ...
        <div style={{ background: 'url(/assets/cielo.png) center / cover no-repeat', padding: 'clamp(12px, 2.5vw, 24px)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'start', background: 'var(--white)' }}>
          ...
          {/* HISTORY — most recent first, regardless of underlying storage order */}
          <div>
            <div style={{ ...SUB_LABEL, padding: '14px 28px', borderBottom: '1px solid var(--gray)' }}>
              {reports.length === 0
                ? (L ? 'Segnalazioni recenti' : 'Recent reports')
                : (L ? `Ultime ${Math.min(reports.length, 10)} segnalazioni` : `Last ${Math.min(reports.length, 10)} reports`)}
            </div>
            {reports.length === 0 ? (
              ...
              [...reports].sort((a, b) => b.dateObj - a.dateObj).slice(0, 10).map((r) => (
                <div key={r.id} style={{ padding: '16px 28px', borderBottom: '1px solid var(--gray)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, fontStyle: 'italic', color: 'var(--gray2)' }}>{r.sensorName ? r.sensorName.replace('Taranto - ', '') : r.district}</span>
                    <span style={{ ...SUB_LABEL, color: 'var(--gray2)', marginLeft: 'auto' }}>{formatReportTime(r.dateObj)}</span>
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {r.symptoms.map((k) => {
